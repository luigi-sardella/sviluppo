...
package LS.Risc_Serbatoio is
   ...
   type Valori_Calcolati is record
      Riscaldamento, Pres_Finale, Temp_Finale, Consumo, Hv : Mio_Float;
   end record;
   type Diam is record
   ...
   Dens_Vap : Mio_Float;
end LS.Risc_Serbatoio;
package body LS.Risc_Serbatoio is
...
--    Cal_Spec : Mio_Float := 4186.0;
   ...
   procedure Esegui is
      ...
      Y_Ini_Ris : constant Positive := 500; --    ""       ""    dei risultati
      ...
      L_Masserb : Label_Type := Label(Fin,(X_Lab,Y_Ini+14*DY),W_Lab,Alt,"Massa di liquido nel serbatoio, kg");
      E_Masserb : Editbox_Type := Editbox(Fin,(X_Edit,Y_Ini+14*DY),W_Edit,Alt,"1000.0");
      L_Temporisc : Label_Type := Label(Fin,(X_Lab,Y_Ini+15*DY),W_Lab,Alt,"Tempo di riscaldamento, min");
      E_Temporisc : Editbox_Type := Editbox(Fin,(X_Edit,Y_Ini+15*DY),W_Edit,Alt,"10.0");
      ...
      Senzascarico : Label_Type := Label(Fin,(X_Edit,Y_Ini_Ris+10*DY),2*W_Lab,Alt,"");
      Massafinale : Label_Type := Label(Fin,(X_Edit,Y_Ini_Ris+11*DY),2*W_Lab,Alt,"");
      Conscarico : Label_Type := Label(Fin,(X_Edit,Y_Ini_Ris+12*DY),2*W_Lab,Alt,"");
      ...
      Testo : String(1..10);
      Massa_Serbatoio, Tempo_Risc, Temper_Iniz : Mio_Float;
   begin
	 ...
	 begin
         case Next_Command is
		  ...
		  Massa_Serbatoio := Mio_Float'Value(Get_Text(E_Masserb));
                  Tempo_Risc := Mio_Float'Value(Get_Text(E_Temporisc));
                  declare
		  ...
		  Dens_Vap := LS.Vapore.Dens_V(Pres_Vap,Temp_Vap);
               ...
               exception
                  when Pressione_Acqua_Negativa =>
                     Show_Message("      Diminuire la portata d'acqua o allargare la gola",
                                  "Pressione dell'acqua negativa");
                  when Pressione_Vapore_Insufficiente =>
                     Show_Message("                                 Aumentare l'una o diminuire l'altra",
                                  "Poca pressione del vapore rispetto all'acqua");
                  when Margine_Sulla_Saturazione_Superato =>
                     Show_Message("Diminuire la pressione del vapore o il numero o il diametro dei fori",
                                  "Troppo riscaldamento rispetto alla pressione");
               end;
	       ...
	       Put(Testo, Ris_Cal.Hv/Cal_Spec 
		     + (Temper_Iniz - Ris_Cal.Hv/Cal_Spec)/(1.0 + 60.0*Tempo_Risc*Ris_Cal.Consumo/Massa_Serbatoio), 1, 0);
	       Set_Text(Senzascarico, "Temperatura nel serbatoio senza scarico : " & Testo & " gradi");
	       Put(Testo, Massa_Serbatoio + Ris_Cal.Consumo*60.0*Tempo_Risc, 1, 0);
	       Set_Text(Massafinale, " e allora la massa finale nel serbatoio risulta : " & Testo & " kg");
	       Put(Testo, Ris_Cal.Hv/Cal_Spec 
		     + (Temper_Iniz - Ris_Cal.Hv/Cal_Spec)*Exp(-60.0*Ris_Cal.Consumo*Tempo_Risc/Massa_Serbatoio), 1, 0);
	       Set_Text(Conscarico, "Temperatura nel serbatoio con scarico : " & Testo & " gradi");
            when 'R' =>
               DN := Positive'Value(Get_Text(Dia_Nom));
               Dia_Int := Diametri(DN).Interno;
...
with Ada.Text_IO; use Ada.Text_IO;
with LS.Risc_Serbatoio; use LS.Risc_Serbatoio;
procedure LS.Riscserbatoio is
begin
   Esegui;
end LS.Riscserbatoio;
