...
def StamDato(variabile,titolo,unita,stampo='si'):
    ...
    if unita == '':
       ParQDx = ParQSx = ''
    if stampo == 'si':
       print "%s %g %s" % ( titolo+" ", valore, ParQSx+unita+ParQDx)
    return valore
...
def Grafico(fx,x1,x2):  # *** Stampa un grafico della funzione fx nell'intervallo x1, x2 ***
  ...
  for j in range(JSCR, 0, -1):
    for i in range(1,ISCR):
      print scr.get(i,j),
    if j == JSCR:
      print "%s %s %5.4g" % (scr.get(ISCR,j), ' Massimo = ', ybig)
    elif j == 1:
      print "%s %s %5.4g" % (scr.get(ISCR,j), ' Minimo = ', ysml)
    else:
...
#
   for i in range(1,N):
      if mulSca:
         print '  CURVA%2i : il 100%% corrisponde a %10.3e  -  %s'%(i, ScaMul[i], ID[i])
      else:
         print '  CURVA%2i  -  %s'%(i, ID[i])
#
   if not mulSca: print "  Il 100%% corrisponde a %10.3e"%(scale)
   print '\n'
#
   if crossX:
...
#
      ...
      print FormatAsc%(vv,LineCar),
      for icurv in range(1,N-1):
         print FormatOrd%(A[j,icurv]),
      print FormatOrd%(A[j,N-1])
...
if __name__ == '__main__':
   ...
   print solve2(funzione_da_annullare,1.0,3.0)
   ...
   while 1:
      xx = float(raw_input('Ascissa 0..10 [999 per finire]:'))
      if xx == 999: break
      print xx, fun(xx)
   ...
   for i in range(1,n+1):
      for j in range(1,n): print a.get(i,j),
      print a.get(i,n)
...
#     comparison with original matrix.
   ...
   for k in range(1,n+1):
      for j in range(1,n+1):
         if jndx.get(j) == k:
           for l in range(1,n): print x.get(j,l),
           print x.get(j,n)
   print '\nLower matrix of the decomposition:'
   for k in range(1,n+1):
      for l in range(1,n): print xl.get(k,l),
      print xl.get(k,n)
   print '\nUpper matrix of the decomposition:'
   for k in range(1,n+1):
      for l in range(1,n): print xu.get(k,l),
      print xu.get(k,n)
   ...
   print '\nTermini noti:'
   for i in range(1,n+1):
      for j in range(1,m): print b.get(i,j),
      print b.get(i,m)
...
#  test results with original matrix
      ...
      for l in range(1,n): print b.get(l,k),
      print b.get(n,k)
